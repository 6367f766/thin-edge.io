...
mod tests {
    ...
    async fn test_cli_sub_basic(qos: Option<&'static str>) -> Result<(), anyhow::Error> {
        let broker = mqtt_tests::test_mqtt_broker_1883();

        let handle = tokio::spawn(async move {
            let mut cmd = Command::cargo_bin("tedge").unwrap();
            cmd.args(&["mqtt", "sub", "atopic"])
        ...
        let publish_handle = tokio::spawn(async move {
            let () = broker.publish("atopic", "some message").await.unwrap();
        });
        let () = publish_handle.await?;
        let () = handle.await?;
