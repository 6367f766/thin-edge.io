...
macro_rules! display_unkown_for {
    ($x:ident, $y:ident) => {
        let other = &$x.other;
    ...
    fn read_file(&self, path: PathBuf) -> Result<TEdgeConfig, TEdgeConfigError> {
        match std::fs::read(&path) {
            Ok(bytes) => {
                ...
                display_unkown_for!(data, path);
                display_unkown_for!(data.device, path, "device");
                display_unkown_for!(data.c8y, path, "c8y");
                display_unkown_for!(data.az, path, "az");
                display_unkown_for!(data.mqtt, path, "mqtt");
                display_unkown_for!(data.http, path, "http");
                display_unkown_for!(data.software, path, "software");
                display_unkown_for!(data.tmp, path, "tmp");
                display_unkown_for!(data.logs, path, "logs");
                display_unkown_for!(data.run, path, "run");
